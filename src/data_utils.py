...
def create_data_object():
    ...
    Txnperminute= pd.read_csv(r'/data/Trades-per-minute.csv', sep=',')
    ...
    blocksize = pd.read_json(r'/data/block-size-mean.json')
    blocksize.t= blocksize.t.apply(lambda x:dt.datetime.strptime(x, "%Y-%m-%dT%H:%M:%SZ"))
    #print(blocksize)

    with open(r'/data/hash-rate.json') as f1:
        d1=json.load(f1)
    ...
    with open(r'/data/difficulty.json') as f1:
        d2=json.load(f1)
    ...
    Timebb = pd.read_json(r'/data/Time between blocks.json')
    Timebb.t= Timebb.t.apply(lambda x:dt.datetime.strptime(x, "%Y-%m-%dT%H:%M:%SZ"))
    #print('Timebb',Timebb)

    #Je l'ai enlevé faute de donnée recente.
    # TXNnum = pd.read_json(r'C:\Users\User\Desktop\/data\transaction-count.json')
    # TXNnum.t= TXNnum.t.apply(lambda x:dt.datetime.strptime(x, "%Y-%m-%dT%H:%M:%SZ"))
    #print('TXNnm',TXNnum)

    TXNconfirmed= pd.read_csv(r'/data/Confirmed transactions per day.csv', sep=',')
    ...
    mempooltxn= pd.read_csv(r'/data/mempool-count.csv', sep=',')
